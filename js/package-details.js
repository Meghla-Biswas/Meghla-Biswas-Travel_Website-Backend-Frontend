...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const infoSubmitBtn = infoModal
    ? infoModal.querySelector(".submit-info")
    : null;
  const securityConfirmBtn = securityModal
    ? securityModal.querySelector(".final-ok")
    : null;
  ...
  function calculateCost() {
    ...
    if (cbTour.checked)
      servicesList.appendChild(newListItem("City Tour & Guidance"));
    if (cbTransport.checked)
      servicesList.appendChild(newListItem("Airport Transport"));
  }
  ...
  bookBtn.addEventListener("click", (e) => {
    ...
    confirmStart.textContent = adjustStartInput.value
      ? new Date(adjustStartInput.value).toDateString()
      : "Not selected";
    const endDate = adjustStartInput.value
      ? new Date(adjustStartInput.value)
      : null;
    if (endDate) {
    ...
    confirmRoom.textContent =
      roomTypeSelect.value.charAt(0).toUpperCase() +
      roomTypeSelect.value.slice(1);
    confirmCost.textContent = totalCostEl.innerText;

    confirmServices.innerHTML = "";
    if (cbBreakfast.checked)
      confirmServices.appendChild(newListItem("Breakfast"));
    if (cbDinner.checked) confirmServices.appendChild(newListItem("Dinner"));
    if (cbTour.checked)
      confirmServices.appendChild(newListItem("City Tour & Guidance"));
    if (cbTransport.checked)
      confirmServices.appendChild(newListItem("Airport Transport"));
...
if (infoSubmitBtn) {
    infoSubmitBtn.addEventListener("click", function(e) {
        e.preventDefault(); 
     ...
     if (nameInput.value.trim() === "" || emailInput.value.trim() === "" || (passInput && passInput.value.trim() === "")) {
    alert("Please fill in all required fields.");
    return;
}
        
        document.getElementById("db-city").value = city; 
        document.getElementById("db-date").value = document.getElementById("adjust-start").value;
        document.getElementById("db-duration").value = document.getElementById("adjust-days").value;
        document.getElementById("db-people").value = document.getElementById("num-people").value;
        document.getElementById("db-room").value = document.getElementById("room-type").value;
        document.getElementById("db-total").value = document.getElementById("total-cost").innerText;
...
if (congratsOkBtn) {
  ...
  window.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      ...
      if (congratsModal.classList.contains("show")) {
        congratsModal.classList.remove("show");
      }
